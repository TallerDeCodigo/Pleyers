...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_posts($field, $meta) {
    ...
    $posts = new WP_Query($options['args']);
    
    // checkbox_list
    if ('checkbox_list' == $options['type']) {
      if($posts->have_posts()): while($posts->have_posts()): $posts->the_post(); setup_postdata($post); 
        echo "<input type='checkbox' name='{$field['id']}[]' value='$p->ID'" . checked(in_array(get_the_ID(), $meta), true, false) . " /> $p->post_title<br/>";
