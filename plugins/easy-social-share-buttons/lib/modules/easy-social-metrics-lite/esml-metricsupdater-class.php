...
class EasySocialMetricsUpdater {
	...
	public function __construct($options = false) {
		...
		// Set options
		$this->options = ($options) ? $options : get_option(EasySocialShareButtons::$plugin_settings_name);
		...
		// Set up event hooks
		add_action( 'easy_social_metrics_full_update', array( $this, 'scheduleFullDataSync' ) );
	...
	*/
	public function checkThisPost($post_id = 0) {
		...
		// Check TTL timeout
		$last_updated = get_post_meta($post_id, "easy_socialcount_LAST_UPDATED", true);
	...
	*/
	public static function scheduleFullDataSync() {
		...
		foreach ($querydata as $querydatum ) {
			...
			$last_updated = get_post_meta($querydatum->ID, "easy_socialcount_LAST_UPDATED", true);
			$ttl = $options['esml_ttl'] * 3600;
			//if (ESSB_ESML_DEBUG) {
			//	$ttl = 0;
			//}
	...
	public static function removeAllQueuedUpdates() {
		...
		if ( !empty( $crons ) ) {
			foreach( $crons as $timestamp => $cron ) {
				...
				if ( ! empty( $cron['easy_social_metrics_full_update'] ) )  {
					unset( $crons[$timestamp]['easy_social_metrics_full_update'] );
				}
			}
			_set_cron_array( $crons );
			
			wp_clear_scheduled_hook('easy_social_metrics_full_update');
